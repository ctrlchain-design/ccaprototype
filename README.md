...
| `nav.js` | Makes the sidebar navigate between prototypes — Finance from the Orders list opens rate-configuration. |
...
**The sidebar navigates between prototypes.** `_shared/nav.js` maps nav labels to
screens, so from the Orders list you can click Finance and land in
rate-configuration. It matches on the label a designer sees rather than on
markup, because the prototypes were built at different times and their sidebars
have nothing structurally in common. Add a line to its `NAV` map when a prototype
covers a new part of the product.
